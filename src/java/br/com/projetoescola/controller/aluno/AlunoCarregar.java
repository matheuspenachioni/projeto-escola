...
public class AlunoCarregar extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try{
            ...
            AlunoDAOImpl oAlunoDAOImpl = new AlunoDAOImpl();
            request.setAttribute("aluno", oAlunoDAOImpl.carregar(idAluno));
            
            TurmaDAOImpl oTurmaDAOImpl = new TurmaDAOImpl();
            request.setAttribute("turmas", oTurmaDAOImpl.listar());
            request.getRequestDispatcher("/cadastros/aluno/alunoCadastrar.jsp").forward(request, response);
        } catch (Exception ex){
            System.out.println("Problemas na servlet AlunoCarregar! Erro: "+ ex.getMessage());
            ex.printStackTrace();
        }
    }
